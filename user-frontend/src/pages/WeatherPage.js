    ...
    <Box>
      <Box sx={{ textAlign: 'center', mb: { xs: 3, md: 4 } }}>
        ...
        <Typography variant="h6" color="text.secondary" sx={{ fontWeight: 400, fontSize: { xs: '0.875rem', sm: '1rem', md: '1.25rem' } }}>
          Current Conditions & Forecasts for Your Safety
        </Typography>
      </Box>
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    • Stay hydrated with plenty of water<br/>
                    • Wear light, loose-fitting clothing<br/>
                    • Avoid strenuous activities during peak hours<br/>
                ...
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    • Carry an umbrella or raincoat<br/>
                    • Avoid walking through flooded areas<br/>
                    • Stay indoors during thunderstorms<br/>
          ...
          <Card className="dashboard-card">
            <CardContent>
              ...
              {weatherData && weatherData.alerts ? (
                <Box>
                  {weatherData.alerts.slice(0, 3).map((alert, index) => (
                    <Box key={index} mb={2} p={2} 
                         sx={{ 
                           border: '1px solid',
                           borderColor: alert.severity === 'severe' ? 'error.main' : 'warning.main',
                           borderRadius: 2,
                           backgroundColor: alert.severity === 'severe' ? 'error.light' : 'warning.light'
                         }}>
                      <Typography variant="body2" fontWeight="bold" color={alert.severity === 'severe' ? 'error.dark' : 'warning.dark'}>
                        {alert.event}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        {alert.description}
                      ...
                      <Typography variant="caption" color="text.secondary" display="block">
                        Valid until: {new Date(alert.end).toLocaleString()}
                      </Typography>
                    </Box>
                  ))}
                </Box>
