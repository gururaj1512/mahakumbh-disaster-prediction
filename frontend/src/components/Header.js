        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexGrow: 1 }}>
          ...
          <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
            ...
            <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.8)', display: 'block' }}>
              Divine Protection & Safety Management
            </Typography>
          </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.5, sm: 1 } }}>
          ...
          {dashboardData.risk_score && (
            <Box sx={{ display: { xs: 'none', lg: 'block' } }}>
              <Tooltip title={`Overall Risk Score: ${dashboardData.risk_score.overall_score || 0}`}>
                <Chip
                  icon={<WarningIcon />}
                  label={`Risk: ${dashboardData.risk_score.risk_level || 'low'}`}
          ...
          {/* All Alerts */}
          <Tooltip title={`${alerts.length || 0} total alerts`}>
            <IconButton color="inherit" size="small">
              <Badge badgeContent={alerts.length || 0} color="warning">
                <NotificationsIcon />
              </Badge>
            </IconButton>
          </Tooltip>
